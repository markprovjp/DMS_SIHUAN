import { describe, test, expect, vi } from "vitest";
import { OrdersService } from "../orders.service";
import { OrdersController } from "../orders.controller";

describe("Orders Service & Controller", () => {
  test("approve updates order status and creates audit log", async () => {
    const mockPrisma: any = {
      order: {
        findUnique: vi.fn().mockResolvedValue({ id: "o-1", code: "DH01", status: "PENDING" }),
        update: vi
          .fn()
          .mockResolvedValue({ id: "o-1", code: "DH01", status: "APPROVED" }),
      },
      auditLog: {
        create: vi.fn().mockResolvedValue({}),
      },
    };

    const service = new OrdersService(mockPrisma);
    const result = await service.approve("o-1", "user-123", "127.0.0.1");

    expect(mockPrisma.order.findUnique).toHaveBeenCalledWith({
      where: { id: "o-1" },
    });
    expect(mockPrisma.order.update).toHaveBeenCalledWith({
      where: { id: "o-1" },
      data: { status: "APPROVED" },
    });
    expect(mockPrisma.auditLog.create).toHaveBeenCalledWith({
      data: {
        userId: "user-123",
        ipAddress: "127.0.0.1",
        action: "ORDER_APPROVE",
        details: "Đã duyệt đơn hàng mã DH01 (ID: o-1)",
      },
    });
    expect(result.status).toBe("APPROVED");
  });

  test("reject updates order status and creates audit log", async () => {
    const mockPrisma: any = {
      order: {
        findUnique: vi.fn().mockResolvedValue({ id: "o-1", code: "DH01", status: "PENDING" }),
        update: vi
          .fn()
          .mockResolvedValue({ id: "o-1", code: "DH01", status: "REJECTED" }),
      },
      auditLog: {
        create: vi.fn().mockResolvedValue({}),
      },
    };

    const service = new OrdersService(mockPrisma);
    const result = await service.reject("o-1", "user-123", "127.0.0.1");

    expect(mockPrisma.order.findUnique).toHaveBeenCalledWith({
      where: { id: "o-1" },
    });
    expect(mockPrisma.order.update).toHaveBeenCalledWith({
      where: { id: "o-1" },
      data: { status: "REJECTED" },
    });
    expect(mockPrisma.auditLog.create).toHaveBeenCalledWith({
      data: {
        userId: "user-123",
        ipAddress: "127.0.0.1",
        action: "ORDER_REJECT",
        details: "Đã từ chối đơn hàng mã DH01 (ID: o-1)",
      },
    });
    expect(result.status).toBe("REJECTED");
  });

  test("controller endpoints call service approve and reject methods", async () => {
    const mockService: any = {
      approve: vi.fn().mockResolvedValue({ id: "o-1", status: "APPROVED" }),
      reject: vi.fn().mockResolvedValue({ id: "o-1", status: "REJECTED" }),
    };

    const controller = new OrdersController(mockService);
    const mockReq = { user: { id: "user-123" }, ip: "127.0.0.1" };

    const approveResult = await controller.approve("o-1", mockReq);
    expect(mockService.approve).toHaveBeenCalledWith("o-1", "user-123", "127.0.0.1");
    expect(approveResult.status).toBe("APPROVED");

    const rejectResult = await controller.reject("o-1", mockReq);
    expect(mockService.reject).toHaveBeenCalledWith("o-1", "user-123", "127.0.0.1");
    expect(rejectResult.status).toBe("REJECTED");
  });
});
